...
export default function useAddLikeStorage() {
    ...
    const toggleLike = useCallback((likeValue: TokenInfo) => {
        const isLiked = storedLikes.some((like: TokenInfo) => like.tokenName === likeValue.tokenName && like.tokenSymbol === likeValue.tokenSymbol);
    
        let updatedLikes;
        if (isLiked) {
            updatedLikes = storedLikes.filter((like: TokenInfo) => like.tokenName !== likeValue.tokenName || like.tokenSymbol !== likeValue.tokenSymbol);
        } else {
